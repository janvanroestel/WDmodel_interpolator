...
class WD_DBmodels:
    ...
    def __init__(self,filedir,
        ...
        # get T and logg values
        T = np.array([float(os.path.basename(f)[3:8]) for f in files])
        logg = np.array([float(os.path.basename(f)[9:12]) for f in files])*0.01
        ...
        def loadspec(f):
            data = np.loadtxt(f)
            # convert from F_l to F_nu
            data[:,1] = data[:,1] * 10**26 * (data[:,0]*10**-10)**2/(3*10**8)
            return data

        modelspectra = [loadspec(f) for f in files]
        N = np.array([np.size(d[:,0]) for d in modelspectra])
